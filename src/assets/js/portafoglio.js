(function (global) {
  ...
  function gbmMonthlyMultiplier(asset, options) {
    ...
    const mu = macroAdjustedAnnualReturn(asset, options && options.macroState, options && options.enableMacroScenario);
    const dt = 1 / 12;
    const z = normalRandom(options.random);
  ...
  function rebalancePortfolio(portfolio, allocation) {
    const total = Object.keys(portfolio).reduce((sum, asset) => sum + portfolio[asset], 0);
  ...
  function calculateContribValue(state, month) {
    return V.toNumber(state.initialInvestment, 0) + V.toNumber(state.monthlyContribution, 0) * Math.max(0, Number(month) || 0);
  }
  function simulatePortfolioPath(options) {
    const settings = { ...options, allocation: { ...options.allocation } };
    const months = Math.max(0, Math.round(V.toNumber(settings.timeHorizonYears, 0) * 12));
    const random = settings.random || createPrng(settings.seed);
    ...
    for (let month = 1; month <= months; month += 1) {
      ...
      const nominal = Object.values(portfolio).reduce((sum, value) => sum + value, 0);
  ...
  global.gbmReturnsByMonth = gbmReturnsByMonth;
})(window);
